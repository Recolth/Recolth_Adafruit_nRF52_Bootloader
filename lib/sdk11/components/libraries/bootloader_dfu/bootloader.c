...
typedef enum {
    ...
    BOOTLOADER_COMPLETE, /**< Bootloader status for indicating that all operations for the update procedure has completed and it is safe to reset the
                            system. */
...
static pstorage_handle_t m_bootsettings_handle; /**< Pstorage handle to use for registration and identifying the bootloader module on subsequent calls to
                                                   the pstorage module for load and store of bootloader setting in flash. */
static bootloader_status_t
    m_update_status; /**< Current update status for the bootloader module to ensure correct behaviour when updating settings and when update completes. */
static bool m_cancel_timeout_on_usb; /**< If set the timeout is cancelled when USB is enumerated. Otherwise, the timeout is only cancelled when DFU update
                                        is started. */
...
static void wait_for_events(void) {
    for (;;) {
        ...
        if ((m_update_status == BOOTLOADER_COMPLETE) || (m_update_status == BOOTLOADER_TIMEOUT) || (m_update_status == BOOTLOADER_RESET)) {
...
bool bootloader_app_is_valid(void) {
    ...
    if (*((uint32_t *)app_addr) == EMPTY_FLASH && *((uint32_t *)(app_addr + 4)) == EMPTY_FLASH) {
        return false;
    }
    ...
    if (p_bootloader_settings->bank_0 == BANK_VALID_APP) {
        ...
        if (p_bootloader_settings->bank_0_crc != 0) {
            image_crc = crc16_compute((uint8_t *)app_addr, p_bootloader_settings->bank_0_size, NULL);
        }
...
bool bootloader_dfu_sd_in_progress(void) {
    ...
    if (p_bootloader_settings->bank_0 == BANK_VALID_SD || p_bootloader_settings->bank_1 == BANK_VALID_BOOT) {
        return true;
    }
...
uint32_t bootloader_dfu_sd_update_continue(void) {
    ...
    if ((dfu_sd_image_validate() == NRF_SUCCESS) && (dfu_bl_image_validate() == NRF_SUCCESS)) {
        return NRF_SUCCESS;
    }
